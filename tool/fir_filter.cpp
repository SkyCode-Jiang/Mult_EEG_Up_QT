...
FIR_filter::FIR_filter( int taps, double f1, double f2, QString type,
                        QString window): h(taps, 0), samples(taps, 0)
{
    this->idx     = 0;
    this->taps    = taps;
    ...
    if (!QString::compare(type, "lp")) {
        h = lowPass_coefficient(taps, f1);
    }
    else if (!QString::compare(type, "hp")) {
        h = highPass_coefficient(taps, f1);
    }
    else if (!QString::compare(type, "bp")) {
        h = bandPass_coefficient(taps, f1, f2);
    }
    else if (!QString::compare(type, "sb")) {
        h = bandStop_coefficient(taps, f1, f2);
    }

    // Calculate the window to improve the FIR filter
    if (!QString::compare(window, "hamming")) {
        w = window_hammig(taps);
    }
    else if (!QString::compare(window, "triangle")) {
        w = window_triangle(taps);
    }
    else if (!QString::compare(window, "hanning")) {
        w = window_hanning(taps);
    }
    else if (!QString::compare(window, "blackman")) {
        w = window_blackman(taps);
    }

    if (!QString::compare(window, "")) {
        this->h = h;
    }
    else
    {
...
double FIR_filter::filter(double new_sample)
{
    ...
    // Save the new sample 保存新的输入样本
    this->samples[this->idx] = new_sample;

    // Calculate the output  卷积计算输出
